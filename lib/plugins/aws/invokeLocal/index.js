...
class AwsInvokeLocal {
  ...
  invokeLocal() {
    ...
    if (runtime === 'go1.x') {
      const localBinPath = process.env.SLS_GO_BIN_LOCAL || 'bin-local'
      let localHandler = handler.replace('bin', localBinPath)
      // don't need separate binaries for linux - GOOS=linux already
      if(os.platform() === 'linux') {
        localHandler = handler
      }
      return this.invokeLocalGo(
    ...
    throw new this.serverless.classes
      .Error('You can only invoke Node.js, Python, Java & Go functions locally.');
  }
  ...
  callGoBridge(bridgePath, handlerPath, input) {
    return new BbPromise((resolve) => fs.statAsync(handlerPath).then(() => {
      const bridge = spawn(bridgePath, [handlerPath], {
        cwd: path.join(__dirname, 'go'),
        shell: true
      });

      bridge.stdout.on('data', (buf) => this.serverless.cli.consoleLog(buf.toString()));
      bridge.stderr.on('data', (buf) => this.serverless.cli.consoleLog(buf.toString()));
  ...
  invokeLocalGo(servicePath, handler, event, context) {
    const input = JSON.stringify({
      event: event || {},
      context,
    });
    const bridgePath = path.join(__dirname, 'go', 'bridge')
    const handlerPath = path.join(servicePath, handler)
    return new BbPromise(resolve => fs.statAsync(bridgePath)
      .then(() => this.callGoBridge(bridgePath, handlerPath,  input))
      .then(resolve)
      .catch(() => {
        const make = spawn('make', [
          'build-bridge',
        ], { cwd: path.join(__dirname, 'go'), shell: true });

        this.serverless.cli
          .log('Building Go bridge, first invocation might take a bit longer.');

        make.stderr.on('data', (buf) => this.serverless.cli.consoleLog(`make - ${buf.toString()}`));
        make.stdin.end();

        make.on('close', () => this.callGoBridge(bridgePath, handlerPath, input)
          .then(resolve));
      }));
  }
}
